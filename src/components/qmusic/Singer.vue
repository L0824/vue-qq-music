              ...
              <transition-group tag="ul" name="fade-in">
                <li class="singer_list_item" v-for="(item,index) in topSingerArr" @click="selectSinger(item)" :key="item">
                  <div class="singer-box">
                    <!-- 在装载文档或图像的过程中如果发生了错误，就会触发onerror事件。可以使用一张提示错误的图片代替显示不了的图片 -->
                    <!-- Fsinger_mid 歌手图片的ID -->
                    <img
    ...
    computed: {
      ...
      topSingerArr(){
        if (this.singerObj.list) {
          // this.singerObj.list 有100条数据 截取到0-10 sliceNum:10
      ...
      singerArr(){
        if (this.singerObj.list) {
          // console.log(this.singerObj.list)
          // 从第10条开始截取
