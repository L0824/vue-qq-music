        ...
        if(!this.isDigital){
          ...
          if(this.swiperList.length>0){
            var mySwiper = new Swiper('.swiper-container-digital', {
              ...
              coverflow: {
                // slide做3d旋转时Y轴的旋转角度。默认50
                rotate: 30,
                // 每个slide之间的拉伸值，越大slide靠得越紧。 默认0。
                stretch: 10,
                // slide的位置深度。值越大z轴距离越远，看起来越小。 默认100。
                depth: 60,
                // depth和rotate和stretch的倍率，相当于depth*modifier、rotate*modifier、stretch*modifier，值越大这三个参数的效果越明显。默认1。
                modifier: 2,
                // 开启slide阴影。默认 true。
                slideShadows : true
              },
