      ...
      <div class="home-wrapper">
        ...
        <div class="hot-page">
          ...
          <div class="hot-wrapper">
            <v-swiper v-if="shoufaData" :swiperList="firstData.hotdiss.list" :isFormat="!isFormat" :duration="6000"></v-swiper>
          </div>
        </div>
        <div class="top-page">
          <h3 class="page-title">
          ...
          <div class="top-wrapper">
            <ul class="toplist-wrapper">
              <li v-for="(item,index) in firstData.toplist" v-bind:key="index">
                ...
                <ul class="toplist-songlist">
                  <li v-for="(song,idx) in item.songlist" v-bind:key="idx">
                    <p>{{idx+1}} <a>{{song.songname}}</a></p>
                    <p class="singer-name" @click="toSinger(song,false)"><a>{{song.singername}}</a></p>
                  </li>
                </ul>
              </li>
            </ul>
        ...
        <div class="mv-page">
          ...
          <div class="shoufa-wrapper">
            ...
            <div class="shoufa-nav">
              <span v-for="(nav,key) in shoufaData" v-bind:key='key' @click="changeArea(key)" :class="{'active' :key==areaKey}">{{key=='all'?'全部':key=='neidi'?'内地':key=='gangtai'?'港台':key=='oumei'?'欧美':key=='japan'?'日本':key=='korea'?'韩国':key}}</span>
            </div>
...
<script>
  ...
  import Swiper from './qmusic/Swiper'
  export default{
    data(){
      return {
        firstData:{},
